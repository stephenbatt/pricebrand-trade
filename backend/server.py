...
def background_worker():
    ...
    while True:
        try:
            ...
            # Only trigger around 4:00 PM EST on weekdays, once per day
            is_close_time = is_weekday and hour == 16 and minute < 5

            if is_close_time and not already_settled_today:
                print("Market close detected — settling trades")
                try:
                    # Call the existing FastAPI endpoint so it runs in the main event loop
                    with httpx.Client(timeout=30.0) as client_http:
                        client_http.post("http://127.0.0.1:8000/api/auto-settle")
                except Exception as e:
                    print(f"Error calling auto-settle endpoint: {e}")
                already_settled_today = True

            # Reset flag earlier in the day so next day can settle again
            if hour < 15:
                already_settled_today = False
